...
type ExpandDotKeys<T extends object> = {
  [K in TopLevelKeys<keyof T> & string]?: HasDirectKey<T, K> extends true
    ? HasNestedKey<T, K> extends true
      ? Prettify<ExpandDotKeys<GatherByPrefix<T, K>> & { DEFAULT?: T[K & keyof T] }>
      : T[K & keyof T]
...
export function buildSyntax(obj: Record<string, string | SyntaxConfig>): Record<string, HighlightStyleContent> {
